\chapter{Anatomizing Smartphone Application Performance} \label{chap:app}
...
%%briefly take about methodology
\iffalse
...
\ncaption{Script execution speed for different platforms}
\label{fig:web_js}
\end{figure}

%compression
\nsubsection{Content compression} \label{sec:web_compress}
...
We measured Javascript execution time on different phones using a
...
%In order to
%improve client execution speed, OS vendors as well as hardware
%vendors should make joint optimizations.


%Server configuration and content optimization
\nsubsection{Server configuration \& content optimization}
...
%breakdown
\nsubsection{Web browsing via 3G networks}
\label{sec:web_real}
...
%the speed difference in HTML rendering and JavaScript execution. This
%is confirmed by noting that the client execution speed ranking shown
%in  is consistent with the Web browsing
%performance ranking for WiFi.

%sum for Web browsing section
\paragraph{Summary:} First, we find that higher browser concurrency 
