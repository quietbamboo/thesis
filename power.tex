...
%original energy
\MR{Traffic type} & Payload    & \% of downlink  & \# of  packets & \% of  downlink & Avg downlink packet & Avg uplink packet\\
 &  (GB) / \%$^{a}$ &  payload & ($\times10^{6}$) / \%$^{b}$ &  packets &   payload size (B) &   payload size (B)  \\\hline
...
\MR{Process name} & Off payload    & \% of downlink  & \# of off packets & \% of  downlink & Avg downlink off packet & Avg uplink off packet\\
 &  (GB) / \%$^{c}$ & off payload$^{d}$ & ($\times10^{6}$) / \%$^{e}$ & off packets$^{f}$ &   payload size (B) &   payload size (B)  \\\hline
...
\end{tabular}
\begin{tabular}{l}
\\{\small $^a$ Payload refers to the total screen-on/off payload, and \% is relative to the total payload of all traffic.}
\\{\small $^b$ \% relative to the total number of packets of all traffic.}
\\{\small $^c$ Off payload refers to the screen-off payload of the specific application, and \% is relative to the total payload of this application.}
\\{\small $^d$ \% of downlink screen-off payload of the specific application relative to the total screen-off payload of that application.}
\\{\small $^f$ \% of downlink screen-off packet count of the specific application relative to the total screen-off packet count of that application.}
\\{\small $^g$ Full process name: /data/data/com.skype.raider/files/skypekit, which is not the actual Skype application (com.skype.raider).}
\end{tabular}
\ncaption{Packet characteristics of screen-on/off traffic and top processes for screen-off traffic}
\label{tab:packet}
\end{table*}
...
%{\bf XXX Overall This subsection is confusing and missing a strong take home message. Details below}

Using the methodology described in Section~\ref{sec:data.umich}, we classify all packets to be screen-on, screen-off or unknown.
%, if the packet does not belong to any screen session. 
Table~\ref{tab:stats} lists the number of packets in different categories. 
Among the total 131.49 million packets, 55.12\% of them are screen-on packets and 35.85\% are screen-off packets, with 9.02\% unknown. The possible reasons for unknown packets are multifold, including that voluntary users may have accidentally killed the data collector. %, or some subtle software bugs within our data collector. 
For the unknown category, we conservatively choose not optimize it for either screen-on or screen-off traffic optimization. 
%Ideally, with the screen status of this traffic, our proposed optimization could have better effects by additionally optimizing the unknown traffic.
%{\bf XXX Do you really ignore it ?You dont eliminate from the trace,  do you ? If that is correct, give a more accurate description of how unknown traffic is treated in the analysis. Maybe you dont characterize the traffic in this section. What about when you do the optimizations ? }
...
%The gap for payload is larger than that of packet number, which can be explained by that screen-off traffic has less downlink packets (52.60\% $<$ 60.71\% for screen-on) which has larger size, and for downlink packets, the average size for screen-off traffic is smaller than that of screen-on (823.20B $<$ 1126.43B).

To understand individual process behavior within screen-off traffic, we scrutinize the top processes 
%{\bf XXX are these processes or applications ?} %of screen-off traffic, 
sorted by the number of screen-off packets, in the bottom section of Table~\ref{tab:packet}. In the second column, titled ``Off payload'', we observe that some processes have most of their payload transferred during screen-off sessions, \eg Genie Widget, Epicurious Recipe, \etc Especially for Yahoo! Sportacular, 80.45\% of all its payload is transferred when the screen is off. 
This is possibly due to the background behaviors of these processes, involving either periodically pulling updates from servers or traffic triggered by server-initiated push notifications. 
%{\bf XXX The previous sentence - are these facts that you know or just likely causes that you think-if the latter, mend the language accordingly. Are there only these 2 types of causes ? }
In terms of the \% of downlink payload, compared to other processes, Skypekit, Gmail, Sportacular and Facebook have smaller proportions of  their respective total downlink payloads associated with the screen-off states. 
%This observation is further supported by the fact that 
These processes also have a smaller average downlink packet size ($<$500B). In contrast, a process like \hd{android.process.media} has an average size of 1388B, indicating that most packets have a size of MTU (maximum transmission unit, $\sim$1500B). This shows that for screen-off traffic, application behavioral diversity still exists similar to that of screen-on traffic.
%This suggests that some processes are downloading objects with a relatively large size (no smaller than MTU which is roughly 1.4$\sim$1.5KB), while other processes are communicating with remote hosts using protocols that have much smaller downlink payloads.
%{\bf XXX The last sentence seems obvious given the data. What is the big-picture  message from the past few sentences about different packet sizes? Is there anything insightful?  Also there seems to be an overlap between the observations above and the payload discussion below.  Can you streamline the text? }
%especially, \eg, for Skypekit, the average downlink packet payload size is only 22B and this type of traffic is highly unlikely for downloading any object or for audio/video streaming.

A group of processes share quite similar behavior patterns, including Google Music, \hd{/system/bin/mediaserver} and \\\hd{android.process.media}, which have larger payload ($>$2GB), and $>$99\% of the payload is downlink. Their average downlink packet sizes are close to the MTU and average uplink packet sizes are close to 0. 
%We find that most of their downlink packets are MTU-sized data packets and most of their uplink packets are 0-sized TCP ACK packets. {\bf XXX Are the last 2 sentences saying the same thing? Can we combine them ? } 
%This is further verified by {\bf XXX What is further verified ? Why do you need further verification? Did'nt you actually measure the numbers above?} 
Also, their ratio of downlink packets is close to 2/3. This is because TCP's delayed ACK would generate one uplink ACK packet for every two downlink data packets, resulting in a ratio of 2/3 of downlink packets. 
%{\bf  XXX I am confused. What is the point we are trying to make here?  Are you trying to explain why the packets in 1 dirn are full and  the other ack ? or that these are content download type apps ? If so why do you care that x\% of payload is downstream or not ? } 
These observations suggest that Google Music is downloading large amount of data, as it can run in the background allowing users to listen to the music with the screen off.
%, the traffic generated by Google Music mostly comes from this kind of scenarios. {\bf XXX what kind of scenarios ?}  
Similarly, although \hd{/system/bin/mediaserver} and \hd{android.process.media} are not actual applications, they are used by other applications, such as Pandora, to download contents for users while the screen is off. However, this group of processes does not necessarily have higher energy consumption compared with the remaining processes, and we explore this in more detail in latter sections.

%The above analysis provides detailed understanding of packet characteristics of screen-off traffic and we observe the volume of screen-off traffic is smaller than screen-on traffic.
...
%original energy
\MR{Traffic type} & \MR{\# of bursts} & Avg$^a$ \# of  & Avg$^a$ \# of  & Avg$^a$ uplink  & Avg$^a$ downlink  & Avg$^a$ burst  & Avg$^a$ \IBT\\
 &  & uplink packets & downlink packets & payload (B) & payload (KB) &  length (sec)  & following (sec)\\\hline
 Screen-on & 650,941 & 43.75 & 67.62 & 2910.44 & 76.17 & 2.92 & 335.13\\\hline
Screen-off & 1,910,939  & 11.69 & 12.98 & 739.78 & 10.68 & 1.37 & 113.60\\\hline\hline %id >= 0

\MR{Process name} & \MR{\# of bursts} & Avg$^a$ \# of  & Avg$^a$ \# of  & Avg$^a$ uplink  & Avg$^a$ downlink  & Avg$^a$ burst  & Avg$^a$ \IBT\\
 &  & uplink packets & downlink packets & payload (B) & payload (KB) &  length (sec)  & following (sec)\\\hline
\rowcolor[gray]{.8} Genie Widget & 5,952 & 319.73 & 319.36 & 8852.48 & 287.88 & 17.87 & 3,892.87\\\hline %id 361
\rowcolor[gray]{.8} Google Music & 5,297 & 195.69 & 427.56 & 505.54 & 591.92 & 4.53 & 5,111.50\\\hline %id 178
Epicurious Recipe & 63,236  & 21.07 & 21.46 & 202.22 & 26.01 & 0.67 & 159.34\\\hline %id 64
\rowcolor[gray]{.8} /system/bin/mediaserver & 8,163  & 106.44 & 215.53 & 669.82 & 289.35 & 5.01 & 14,451.70\\\hline %id 16
\rowcolor[gray]{.8} android.process.media & 1,442 & 461.88 & 1,156.93 & 246.99 & 1,605.84 & 19.83 & 123,565.00\\\hline %id 12
Skypekit & 42,744& 25.08 & 23.46 & 555.38 & 0.52 & 1.93 & 832.79\\\hline %id 114
Facebook & 203,535 & 5.49 & 4.07 & 318.83 & 1.98 & 0.86 & 547.23\\\hline %id 52
Yahoo! Sportacular & 133,785  & 8.39 & 6.07 & 285.44 & 1.45 & 1.52 & 261.78\\\hline %id 130
Gmail & 105,478   & 6.60 & 6.02 & 1375.30 & 2.41 & 1.17 & 2,002.60\\\hline %id 13
\end{tabular}
\begin{tabular}{l}
\\{\small $^a$ Each ``avg'' in this table stands for the average value per burst.}
\end{tabular}
\ncaption{Burst analysis of screen-on/off traffic and top processes for screen-off traffic}
\label{tab:burst}
\end{table*}
...
Following the methodology in \S\ref{sec:method.burst}, Table~\ref{tab:burst} lists the results of the burst analysis for screen-off traffic, with that of screen-on traffic listed for comparison purposes.
