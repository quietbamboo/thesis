...
%Given that most bursts are small and occur in user's idling time, we choose to ignore the user inputs and
...
%Among different header fields, we choose the ones that are more possible to have an impact on \IBT.
%\comment{explain a little more why we choose these features, maybe add server IP feature?}
...
%\hline
%$\alpha_u$  & \multicolumn{2}{|c|}{438.39 mW/Mbps} \\
%$\alpha_d$  & \multicolumn{2}{|c|}{51.97 mW/Mbps} \\
%$\beta$     & \multicolumn{2}{|c|}{1288.04 mW} \\
%\hline
\end{tabular}
\begin{tabular}{l}
...
%\MR{\NAME: \NB} & \NB classification with {\em mvmn}:  & \MR{48.0\%} & \MR{102.8\%} & \MR{0.47} \\
\MR{\NAME: \NB} & \NB classification with {\em mvmn}:  & \MR{53.0\%} & \MR{107.9\%} & \MR{0.49} \\
 & multivariate multinomial distribution & &  & \\\hline
%\MR{\NAME: Discriminant} & Discriminant analysis with  & \MR{151.4} & \MR{27.3} & \MR{75.4\%} & \MR{54.7\%} & \MR{118.9\%} & \MR{0.46} \\
% & discriminant type as {\em pseudoLinear} & & & & & &\\\hline
%\NAME: Regression Tree & Binary decision tree for regression & 490.8 & 2.6 & 78.4\% & 57.8\% & 139.6\% & 0.41 \\\hline
%\NAME: Classification Tree & Binary decision tree for classification  & 52.1\% & 101.6\%  & 0.51 \\\hline
\NAME: Classification Tree & Binary decision tree for classification  & 59.1\% & 91.0\%  & 0.65 \\\hline
%\NAME: SVM & SVM with {\em Multilayer Perceptron} kernel & 591.2 & 1.6 & 66.4\% & 45.0\% & 141.2\% & 0.32 \\\hline
%\NAME: Ensemble & Method: {\em RobustBoost}; Error goal: 0.15 & \MR{2534.2} & \MR{412.8} & \MR{81.9\%} & \MR{56.3\%} & \MR{106.3\%} & \MR{0.53} \\
%(RobustBoost) & weak leaner: decision tree; \# of trees: 20 &  &  &  &  &  & \\\hline
%\MR{\NAME: Ensemble (Bag)} & Method: {\em Bag}; type: classification & \MR{56.6\%} & \MR{92.2\%}  & \MR{0.61}\\
...
\paragraph{\NAMEFULL}: we explore various machine learning algorithms with the \MostRecent prediction model ($\alpha=1000$ and $\beta=1$). %The features used for machine learning are listed in Table~\ref{tab:featureSum}. To validate our approach that groups all categories with less than 0.1\% of total bursts for each feature, we use \NB algorithm to compare the prediction accuracy. We observe that the accuracy improves from 68.99\% to 73.20\% after grouping for the features, and this is mainly due to the reduced over fitting problem.
...
\end{tabular}
\ncaption{Prediction performance and accuracy of different machine learning algorithms}
...
\nsubsection{Running Overhead on Real Phone}
