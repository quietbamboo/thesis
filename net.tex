...
In the following of this chapter, we first discuss the design and implementation of \mobiperf in Section~\ref{sec:net.design} and following that, we discuss the measurement results collected via \mobiperf and their implications.
...
server but not locally on the phone (Section~\ref{sec:net.design}). This allows 
...
\nsubsection{Cellular Network Infrastructure and Implications}
...
\begin{figure}[t]
\centering
\IG{figures/srii/dis_handshake_3g.eps}\\
\ncaption{Handshake RTT vs. spherical distances}
...
%For example, for the scenario with 1 CDN server, the user only has one choice.
...
\begin{figure}[t]
\centering
\IG{figures/srii/cdn_handshake.eps}\\
\ncaption{RTT to closest synthetic CDN node}
...
%\item Sprint and Verizon have higher TCP retransmission rate compared with AT\&T and T-Mobile.
...
\nsubsection{Network-based LTE Parameter Inference}
\label{sec:net.sm}

\begin{figure}[t]
\centering
\IG{figures/mobisys12/rtt_drx.eps} \\
\ncaption{LTE RTT v.s. inter-packet idle time}
\label{fig:rtt.drx}
\end{figure}

\textbf{Network-based approach for LTE parameter inference} is used to validate the power-based approach (Section~\ref{sec:power.method}). We use the following experiment to infer LTE state machine and \RI DRX parameters. The LTE phone maintains a long lived TCP connection with a test server. For each experiment, server first sends a packet $P_1$ to trigger UE's promotion to \RC, and after $X$ seconds, server sends another packet $P_2$ to the phone to measure RTT. Assume the RTT at \RC is $RTT_{b}$ ignoring the minor impact of DRX in \RC. When $X\leq T_{tail}$, $RTT(X) = RTT_{b}$. Otherwise, $RTT(X) = RTT_{b} + T_{pro} + T_{drx}$, where $T_{drx} = T_{n} - X$ and $T_{n}$ is the start time of the next DRX on duration after $P_2$'s arrival. If $P_2$'s arrival is inside any DRX on duration, $T_{drx} = 0$.


Figure~\ref{fig:rtt.drx} summarizes the network-based approach for LTE state machine and DRX parameter measurement using the LTE phone. Each point is the median of five runs and for each run, RTT is measured after making the device idle for a specific time. The pattern of RTT is expected as inter-packet idle time changes. Based on these results, we can infer the values of the following parameters in Table~\ref{tab:parameter}: $T_{tail}$ is around 11.5s, $T_{pi}$ is 1.28s and $T_{oni}$ is close to 45ms. In addition, we observe $T_{pro}$ is around 260ms. These measured values agree with those measured from the power-based approach in Table~\ref{tab:power}, cross-validating our measurement methodology.
